...
for (j in c(1:30)){
  ...
  #-- Impute mel_use--#
  pred <- quickpred(d, mincor = 0.2)
  pred
  meth <- make.method(data = d)
  meth
  d <- mice(data = d, m = 1, predictorMatrix = pred, method = meth, maxit = 5, seed=123)
  d <- complete(d)
...
save(shap_means_all, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/shap_mean_any_mel_use_30imp_SENS.Rda")
load("/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/shap_mean_any_mel_use_30imp_SENS.Rda")
...
for (j in c(1:30)){
  ...
  glm_cvxgboost <- data.frame(summary(glm(mel_use ~ sds_p_ss_dims+famhx_ss_moth_prob_prf_p+as.factor(race_ethnicity)+
                                 reshist_addr1_p1vlnt+reshist_addr1_seg_ice_inc_bw+cbcl_scr_syn_thought_r+
                                 asr_scr_somatic_t+cbcl_scr_syn_attention_r+
                                 female+crpbi_y_ss_caregiver, family="binomial",data=d))$coefficients) %>% mutate(imp=j)
  glm_cvxgboost <- tibble::rownames_to_column(glm_cvxgboost, "Variable")
  coefs[[j]] <- glm_cvxgboost
}
...
save(tab1a, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_tab1a_imp_update_Mar9_SENS.Rda")
write.csv(tab1a, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_tab1a_imp_update_Mar9_SENS.csv")
