...
library(mice)
source("/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/code/abcd_mel_use/shap.R")
...
predictors <- complete(Mice,1) %>% 
  dplyr::select(names(complete(Mice))[c(1,2,23:149,152)])

quantile(predictors$reshist_addr1_p1vlnt,c(0.95))
hist(predictors$reshist_addr1_p1vlnt)
...
save(shap_means_all, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/shap_mean_any_mel_use_30imp.Rda")
load("/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/shap_mean_any_mel_use_30imp.Rda")
shap_means_all$Variable[c(1:10)]


#---------------------------------------------------------------------#
#-- Descriptive statistics table for the top 10 selected predictors --#
#---------------------------------------------------------------------#

#-- Merge predictor data with melatonin use data by subject id
d <- merge(x=complete(Mice),y=ph_p_meds_wide,by="src_subject_id",all=T)

vars <- c("sds_p_ss_dims", "famhx_ss_moth_prob_prf_p", "race_ethnicity",
          "reshist_addr1_p1vlnt", "cbcl_scr_syn_thought_r", "reshist_addr1_seg_ice_inc_bw",
          "asr_scr_somatic_t", "cbcl_scr_syn_attention_r", "crpbi_y_ss_caregiver", "pubertal_sex_p")
factorVars <- c("race_ethnicity", "famhx_ss_moth_prob_prf_p", "pubertal_sex_p")

#-- Look at proportions missing across potential predictors
...
write.csv(table_all_print, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/covariate_descriptions.csv")

#---------------------------------------------------------------------#
#--- Run logistic regression for any melatonin use in imputed data ---#
#---------------------------------------------------------------------#

#--- convert to long-from data set to standardize continuous predictors & add a "female" indicator
long <- complete(Mice, action="long", include=TRUE)
long <- long %>%
  #-- Standardize continuous predictors
  dplyr::mutate(across(.cols = c(sds_p_ss_dims,reshist_addr1_p1vlnt,
                                 cbcl_scr_syn_thought_r,asr_scr_somatic_t,
                                 asr_scr_somatic_t,cbcl_scr_syn_attention_r,
...
for (j in c(1:30)){
  ...
  glm_cvxgboost <- glm(mel_use ~ sds_p_ss_dims+famhx_ss_moth_prob_prf_p+as.factor(race_ethnicity)+
                         reshist_addr1_p1vlnt+cbcl_scr_syn_thought_r+reshist_addr1_seg_ice_inc_bw+
                         asr_scr_somatic_t+cbcl_scr_syn_attention_r+
                         crpbi_y_ss_caregiver+female, family="binomial",data=d)
...
tab1a <- merge(x=shap_means_all,y=mel_or_label,by=c("Short_Label","Variable"),all.y=T)
save(tab1a, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_tab1a_imp_update_Mar9.Rda")
write.csv(tab1a, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_tab1a_imp_update_Mar9.csv")
