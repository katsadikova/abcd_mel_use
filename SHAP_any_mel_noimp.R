...
predictors <- data_all %>% 
  dplyr::select(names(data_all)[c(1,2,23:149,152)])

s <- data.frame(summary(predictors))
write.csv(s, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/predictor_summaries_noimp_updated_Jan17.csv")
...
write.csv(predictors_labeled, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/predictors_labeled_noimp.csv")
...
save(imp_vars_fluid, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_SHAP_noimp_UPDATE_jan16.Rda")
load("/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_SHAP_noimp_UPDATE_jan16.Rda")
...
inc_s <- inc %>% 
  mutate(sds_p_ss_dims = scale(sds_p_ss_dims),
         ...
         cbcl_scr_syn_thought_r = scale(cbcl_scr_syn_thought_r),
         asr_scr_somatic_t = scale(asr_scr_somatic_t),
         reshist_addr1_coi_r_he_nat = scale(reshist_addr1_coi_r_he_nat),
         crpbi_y_ss_caregiver = scale(crpbi_y_ss_caregiver),
         inr = scale(inr))


glm_cvxgboost = glm(data=inc_s, mel_use ~ sds_p_ss_dims+famhx_ss_moth_prob_prf_p+reshist_addr1_p1vlnt+
                      as.factor(race_ethnicity)+reshist_addr1_seg_ice_inc_bw+cbcl_scr_syn_thought_r+
...
tab1a <- merge(x=imp_vars_fluid,y=mel_or_label,by=c("Short_Label","Variable"),all.y=T)
save(tab1a, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_tab1a_noimp_update_Jan16.Rda")
write.csv(tab1a, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_use_tab1a_noimp_update_Jan16.csv")
