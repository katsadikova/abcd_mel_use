...
write.csv(s, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/predictor_summaries.csv")
...
bl_use <- data_all %>% dplyr::select(src_subject_id,mel_use_0,mel_use_1,mel_use_2,mel_use_3,mel_use_4)
...
save(imp_vars_fluid, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_incident_use_SHAP_noimp_update_Jan16.Rda")
...
glm_cvxgboost = glm(data=inc, formula(paste("mel_use ~ ",paste(imp_vars_fluid$Variable[c(1:10)], collapse ="+"), collapse = " ")), family="binomial")
...
inc_s <- inc %>% 
  mutate(sds_p_ss_dims = scale(sds_p_ss_dims),
         ...
         parent_su_comm_risk = scale(parent_su_comm_risk),
         screentime2_p_hours = scale(screentime2_p_hours))

glm_cvxgboost = glm(data=inc_s, mel_use ~ sds_p_ss_dims+famhx_ss_moth_prob_prf_p+
                      reshist_addr1_p1vlnt+as.factor(race_ethnicity)+
                      reshist_addr1_seg_ice_inc_bw+asr_scr_somatic_t+
                      asr_scr_aggressive_t+parent_su_comm_risk+
                      pubertal_sex_p+screentime2_p_hours, family="binomial")
...
save(tab1b, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_incident_tab1b_noimp_update_Jan16.Rda")
write.csv(tab1b, file="/Users/Kat/Dropbox/HSPH_T32/Potential projects/ABCD melatonin/results/mel_incident_tab1b_noimp_update_Jan16.csv")
